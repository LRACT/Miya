...
class Economy(commands.Cog, name="경제"):
    ...
    async def _wallet(self, ctx, user: typing.Optional[discord.User] = None):
        ...
        if not rows:
            ...
            embed = discord.Embed(title=f"💳 {user}님의 지갑 정보", timestamp=datetime.datetime.utcnow(), color=0x5FE9FF)
            embed.add_field(name="잔여 코인", value=f"{rows[0][1]}개", inline=False)
            embed.add_field(name="곧 더 많은 기능이 찾아옵니다...", value="새로운 기능도 많이 기대해주세요!", inline=False)
