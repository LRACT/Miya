        ...
        if role >= ctx.guild.me.top_role:
            ...
            async with ctx.channel.typing():
                ...
                if result == "SUCCESS":
                    ...
                    await ctx.reply(f"<:cs_settings:659355468992610304> 뮤트 역할을 `{role.name}` 역할로 설정했어요.\n \n*관리자 권한을 가진 유저 및 권한 설정을 통해 메시지 보내기 권한을 승인받은 유저는 뮤트가 적용되지 않아요.*")
                else:
        ...
        async with ctx.channel.typing():
            if what == "공지":
                if channel.type == discord.ChannelType.news:
                    await ctx.reply(f"<:cs_no:659355468816187405> 미야 공지 채널은 Discord 커뮤니티 공지 채널이 아닌 곳에만 설정할 수 있어요.")
                else:
                    follow = self.miya.get_channel(config.NotifyChannel)
                    try:
                        await follow.follow(destination=channel, reason="미야 봇 공지 채널 설정")
                    except discord.Forbidden:
                        await ctx.reply(f"<:cs_no:659355468816187405> 공지 채널 설정은 해당 채널에 웹훅 관리 권한이 필요해요.")
                    else:
                ...
                if value is not None and table is not None:
                    ...
                    if result == "SUCCESS":
                        await ctx.reply(f"<:cs_settings:659355468992610304> {what} 채널을 {channel.mention} 채널로 설정했어요.")
                    else:
        ...
        if not args:
            ...
            async with ctx.channel.typing():
                if args[0] == "켜기":
                    ...
                    if result == "SUCCESS":
                        await ctx.reply(f"<:cs_on:659355468682231810> 링크 차단 기능을 활성화했어요!\n특정 채널에서만 이 기능을 끄고 싶으시다면 채널 주제에 `=무시`라는 단어를 넣어주세요.")
                    else:
        ...
        async with ctx.channel.typing():
            ...
            if value is not None:
                ...
                if result == "SUCCESS":
                    ...
                    await ctx.reply(f"<:cs_settings:659355468992610304> {name} 메시지를 성공적으로 변경했어요!\n이제 유저가 {name} 시 채널에 이렇게 표시될 거에요. : \n{a}")
                else:
