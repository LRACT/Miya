        ...
        if role is not None and role < ctx.guild.me.top_role:
            if role not in member.roles:
                if member.guild_permissions.administrator:
                    await ctx.reply(f"<:cs_no:659355468816187405> 지정한 유저가 서버의 관리자 권한을 가지고 있어 취소되었어요.")
                else:
                    await member.add_roles(role, reason=reason)
                    await ctx.reply(f"<:mute:761151751583301682> **{member}**님을 뮤트했어요.\n사유 : {reason}")
            else:
    ...
    async def _unmute(self, ctx, member: discord.Member, *, reason: typing.Optional[str] = "사유 없음."):
        ...
        if role is not None and role < ctx.guild.me.top_role:
            if role in member.roles:
                ...
                await ctx.reply(f"<:mic:761152232447148042> **{member}**님의 뮤트 상태를 해제했어요.\n사유 : {reason}")
            else:
