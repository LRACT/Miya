        ...
        if not rows:
            ...
            try:
                ...
                if g_result == "SUCCESS" and m_result == "SUCCESS":
                    await webhook.terminal(f"Registered >\nGuild - {ctx.guild.name} ({ctx.guild.id})", "서버 등록 기록", self.miya.user.avatar_url)
                    await ctx.send(f"<:cs_yes:659355468715786262> {ctx.author.mention} 서버 등록이 완료되었어요! 이제 미야의 기능을 사용하실 수 있어요.")
                else:
                    await webhook.terminal(f"Register Failed >\nGuild - {ctx.guild.name} ({ctx.guild.id})\nguilds Table - {g_result}\nmemberNoti Table - {m_result}", "서버 등록 기록", self.miya.user.avatar_url)
