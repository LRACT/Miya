        ...
        if not rows:
            ...
            try:
                ...
                if g_result == "SUCCESS" and m_result == "SUCCESS":
                    ...
                    await ctx.send(
                        f"<:cs_yes:659355468715786262> {ctx.author.mention} 서버 등록이 완료되었어요! 이제 미야의 기능을 사용하실 수 있어요."
                    )
                else:
                    await webhook.terminal(
                    ...
                    await ctx.send(
                        f"<:cs_no:659355468816187405> {ctx.author.mention} 서버 등록 도중에 오류가 발생했어요. 등록을 다시 시도해주세요.\n계속해서 이런 현상이 발생한다면 https://discord.gg/tu4NKbEEnn 로 문의해주세요."
                    )
        else:
            await ctx.send(
                f"<:cs_id:659355469034422282> {ctx.author.mention} 서버가 이미 등록되어 있는 것 같아요.\n등록되지 않았는데 이 문구가 뜬다면 https://discord.gg/mdgaSjB 로 문의해주세요."
            )
